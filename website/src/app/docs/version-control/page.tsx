  ...
  return (
    <>
      ...
      <p className="text-[var(--color-text-secondary)] mb-4">
        Every <code className="text-sm px-1 bg-[var(--color-bg-secondary)] rounded">aim store</code> call to an existing model
...
aim store my-model --format safetensors --file v1.safetensors

# Store an updated version (auto-increments version number)
aim store my-model --format safetensors --file v2.safetensors
...
aim get my-model --output ./latest/

# Get a specific version
aim get my-model --version 1 --output ./v1/`}</CodeBlock>
      ...
      <CodeBlock language="bash">{`aim lineage my-model
# v1 ← (initial)
...
aim cleanup my-model --keep 5`}</CodeBlock>
      ...
      <CodeBlock language="rust">{`use ai_model_vault::vault::Vault;
